...
Библиотека для получения статуса серверов Minecraft на Rust: Java Edition (SLP), Bedrock Edition (RakNet), Query-протокол и Legacy SLP (Minecraft < 1.7). Sync и async API.
...
- Legacy Server List Ping для Minecraft < 1.7 (Beta 1.8–1.6.4)
- Query-протокол (полный stat + облегчённый basic stat)
- Парсинг MOTD с выводом в plain/ANSI/HTML/Minecraft-форматировании
- Резолв SRV-записей с опциональным DNS-кэшем (`DnsInfo` в ответе: A-записи, CNAME, TTL)
- Наблюдаемость кэша через `cache_stats()`
- Batch-пинг множества серверов сразу (`Vec` или `Stream`, per-target таймаут)
...
DNS-информация и статистика кэша:

```rust
if let Some(dns) = &status.dns {
    println!("A-записи: {:?}, CNAME: {:?}, TTL: {}s", dns.a_records, dns.cname, dns.ttl);
}
